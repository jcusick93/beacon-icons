...
if (!figmaAccessToken) {
    throw new Error("Missing environment variable FIGMA_PAT. Please generate a personal access token (PAT) under Figma account settings and pass the value in the FIGMA_PAT environment variable.");
}
if (!fileKey) {
    throw new Error("Missing file key. Please pass the Figma file key via a --file argument. For example, given the file URL https://www.figma.com/design/FILE_KEY/Icon-Playground, pass the following: --file FILE_KEY");
}
if (!outputDirectory) {
    throw new Error("Please pass the path to the output directory relative to the current working directory. For example: --out ./icons");
}
...
const newFileNames = new Set(icons.map((icon) => {
    return `${[

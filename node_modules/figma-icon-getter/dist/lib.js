import { z } from "zod";
function parseIconName(iconName, acc = []) {
...
export async function getFigmaIcons({ figmaAccessToken, fileKey, }) {
    function figmaFetch(url, requestInit) {
    ...
    if (!fileRes.ok) {
        ...
        throw new Error(`An error occurred while fetching Figma file with key ${fileKey}. Unexpected ${fileRes.status} response: ${details}`);
    }
    const iconMetadata = z
        .object({
        componentSets: z.record(z.string(), z.object({
            name: z.string(),
        })),
        components: z
            .record(z.string(), z
            ...
            .transform((components) => Object.entries(components).map(([id, iconMetadata]) => ({
            id,
            ...iconMetadata,
        }))),
    })
    ...
    const iconRefsRes = await figmaFetch(`/images/${fileKey}?ids=${iconMetadata
        .map(({ id }) => id)
        .join(",")}&format=svg`);
    return await z
        .object({
        ...
        .transform(({ images }, ctx) => Promise.all(Object.entries(images).map(async ([id, url]) => {
